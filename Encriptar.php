<?php
	...
	/*for( $i=0; $i<strlen($clave); $i++ ){
		if (!in_array($clave[$i], $alfabeto))
			echo "<script language='javascript'>alert('El caracter ".$clave[$i]." de la CLAVE no esta en el Alfabeto');window.location='Polialfabetico.html'</script>";
	}
	...
	$encripta = '';
	for ($a = 0; $a<strlen($mensaje); $a++){
		for ($i=0; $i<9; $i++){
			for ($j = 0; $j<10; $j++){
				if($mensaje[$a] == $m[ $fila[$i] ][ $columna[$j] ])
					$encripta .= $fila[$i].$columna[$j];
			}
		}
	}
?>
